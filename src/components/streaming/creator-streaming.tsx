...
export function CreatorStreaming({
  ...
  const [existingActiveStream, setExistingActiveStream] =
    useState<VolLivestreamOut | null>(null);
  ...
  // Plan usage monitoring — only active while streaming for free users
  ...
  const recorder = useStreamRecorder({
    ...
    onAutoUploadComplete: (recordingUrl) => {
      console.log(
        "Auto-upload completed, showing success modal and redirecting...",
      );
  ...
  const handleStartStream = useCallback(async () => {
    ...
    try {
      ...
      outputStream
        .getAudioTracks()
        .forEach((t) => pc.addTrack(t, outputStream));
  ...
  const handleReconnectToStream = useCallback(async () => {
    ...
    try {
      ...
      outputStream
        .getAudioTracks()
        .forEach((t: MediaStreamTrack) => pc.addTrack(t, outputStream));
      ...
      const errorMsg =
        err instanceof Error ? err.message : "Failed to reconnect to stream";
      setError(errorMsg);
      setConnectionState("failed");
      teardownPublish();
    } finally {
    ...
    if (networkRecovered && isStreaming) {
      const handleNetworkRecovery = async () => {
        ...
        try {
          ...
          console.error(
            "Failed to check active stream after network recovery:",
            err,
          );
        }
      };
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-slate-300 mb-6">
                Your recording has been uploaded and is now available for
                on-demand replay.
              </p>
                          ...
                          micDevices.map((device) => (
                            <button
                              ...
                              onClick={() => {
                                ...
                                audioSourceManagerRef.current?.setSelectedMicDevice(
                                  device.deviceId,
                                );
              ...
              {!isStreaming && hasActiveStream ? (
                ...
                <div className="mb-4 p-4 bg-sky-500/10 border border-sky-500/30 rounded-lg">
                  ...
                  <p className="text-slate-500 text-xs mt-2">
                    Started:{" "}
                    {new Date(
                      existingActiveStream?.created_at || "",
                    ).toLocaleString()}
                  </p>
                </div>
              ) : (
                ...
                onAddChannel={async (type: ChannelType) => {
                  ...
                  try {
                    if (type === "mic") {
                      ...
                      mixerEngineRef.current.addChannel(
                        `mic-${Date.now()}`,
                        "MIC",
                        "mic",
                        stream,
